...
inline DEVICE real4 crp_delta(real4 vec1, real4 vec2, real4 periodicBoxSize, real4 invPeriodicBoxSize, real4 periodicBoxVecX, real4 periodicBoxVecY, real4 periodicBoxVecZ) {
 ...
 */
inline DEVICE real crp_computeAngle(real4 vec1, real4 vec2) {
 ...
 */
inline DEVICE real4 crp_computeCross(real4 vec1, real4 vec2) {
 ...
 */
KERNEL void crp_computeDonorForces(
...
for (int donorStart = 0; donorStart < NUM_DONORS; donorStart += GLOBAL_SIZE) {
        ...
        if (donorIndex < NUM_DONORS) {
            ...
            d4 = (atoms.w > -1 ? posq[atoms.w] : make_real4(0));
        ...
        for (int acceptorStart = 0; acceptorStart < NUM_ACCEPTORS; acceptorStart += LOCAL_SIZE) {
            ...
            posBuffer[4*LOCAL_ID] = (atoms2.x > -1 ? posq[atoms2.x] : make_real4(0));
            posBuffer[4*LOCAL_ID+1] = (atoms2.y > -1 ? posq[atoms2.y] : make_real4(0));
            posBuffer[4*LOCAL_ID+2] = (atoms2.z > -1 ? posq[atoms2.z] : make_real4(0));
            posBuffer[4*LOCAL_ID+3] = (atoms2.w > -1 ? posq[atoms2.w] : make_real4(0));
            }
            SYNC_THREADS;
            if (donorIndex < NUM_DONORS) {
                for (int index = 0; index < blockSize; index++) {
                    ...
                    real4 a1 = posBuffer[4*index];
                    real4 a2 = posBuffer[4*index+1];
                    real4 a3 = posBuffer[4*index+2];
                    real4 a4 = posBuffer[4*index+3];
                    real4 deltaD1A1 = crp_delta(d1, a1, periodicBoxSize, invPeriodicBoxSize, periodicBoxVecX, periodicBoxVecY, periodicBoxVecZ);
#ifdef USE_CUTOFF
        ...
        if (donorIndex < NUM_DONORS) {
            ...
            if (atoms.w > -1) {
                ATOMIC_ADD(&force[atoms.w], (mm_ulong) ((mm_long) (f4.x*0x100000000)));
                ATOMIC_ADD(&force[atoms.w+PADDED_NUM_ATOMS], (mm_ulong) ((mm_long) (f4.y*0x100000000)));
                ATOMIC_ADD(&force[atoms.w+2*PADDED_NUM_ATOMS], (mm_ulong) ((mm_long) (f4.z*0x100000000)));
                MEM_FENCE;
            }
#else
            ...
            if (atoms.w > -1) {
                unsigned int offset = atoms.w+bufferIndices.w*PADDED_NUM_ATOMS;
                real4 force = forceBuffers[offset];
                force.xyz += f4.xyz;
                forceBuffers[offset] = force;
            }
 ...
 */
KERNEL void crp_computeAcceptorForces(
#ifdef SUPPORTS_64_BIT_ATOMICS
	GLOBAL mm_ulong* RESTRICT force,
#else
    ...
    for (int acceptorStart = 0; acceptorStart < NUM_ACCEPTORS; acceptorStart += GLOBAL_SIZE) {
        ...
        if (acceptorIndex < NUM_ACCEPTORS) {
            ...
            a4 = (atoms.w > -1 ? posq[atoms.w] : make_real4(0));
#ifdef USE_EXCLUSIONS
        ...
        for (int donorStart = 0; donorStart < NUM_DONORS; donorStart += LOCAL_SIZE) {
            ...
            if (acceptorIndex < NUM_ACCEPTORS) {
                for (int index = 0; index < blockSize; index++) {
                    ...
                    real4 d1 = posBuffer[4*index];
                    real4 d2 = posBuffer[4*index+1];
                    real4 d3 = posBuffer[4*index+2];
                    real4 d4 = posBuffer[4*index+3];
                    real4 deltaD1A1 = crp_delta(d1, a1, periodicBoxSize, invPeriodicBoxSize, periodicBoxVecX, periodicBoxVecY, periodicBoxVecZ);
#ifdef USE_CUTOFF
                    if (deltaD1A1.w < CUTOFF_SQUARED) {
#endif
