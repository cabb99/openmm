...
class CommonCalcCustomResiduePairForceKernel::ForceInfo : public ComputeForceInfo {
public:
    ForceInfo(const CustomResiduePairForce& force) : force(force) {
    }
    bool areParticlesIdentical(int particle1, int particle2) {
        return true;
    }
...
private:
    const CustomResiduePairForce& force;
};
...
void CommonCalcCustomResiduePairForceKernel::initialize(const System& system, const CustomResiduePairForce& force) {
    ...
    if (numDonors == 0 || numAcceptors == 0)
        return;
    int numParticles = system.getNumParticles();
    donors.initialize<mm_int4>(cc, numDonors, "customResiduePairDonors");
    acceptors.initialize<mm_int4>(cc, numAcceptors, "customResiduePairAcceptors");
    donorParams = new ComputeParameterSet(cc, force.getNumPerDonorParameters(), numDonors, "customResiduePairDonorParameters");
    acceptorParams = new ComputeParameterSet(cc, force.getNumPerAcceptorParameters(), numAcceptors, "customResiduePairAcceptorParameters");
    if (force.getNumGlobalParameters() > 0)
        globals.initialize<float>(cc, force.getNumGlobalParameters(), "customResiduePairGlobals");
    vector<vector<float> > donorParamVector(numDonors);
    vector<mm_int4> donorVector(numDonors);
    for (int i = 0; i < numDonors; i++) {
        vector<double> parameters;
    ...
    if (!cc.getSupports64BitGlobalAtomics()) {
        donorBufferIndices.initialize<mm_int4>(cc, numDonors, "customResiduePairDonorBuffers");
        acceptorBufferIndices.initialize<mm_int4>(cc, numAcceptors, "customResiduePairAcceptorBuffers");
        vector<mm_int4> donorBufferVector(numDonors);
        vector<mm_int4> acceptorBufferVector(numAcceptors);
        vector<int> donorBufferCounter(numParticles, 0);
        for (int i = 0; i < numDonors; i++)
    ...
    for (int i = 0; i < force.getNumExclusions(); i++) {
        ...
        if (donorExclusionVector[donor].x == -1)
            ...
            throw OpenMMException("CustomResiduePairForce: this platform does not support more than four exclusions per donor");
        if (acceptorExclusionVector[acceptor].x == -1)
            acceptorExclusionVector[acceptor].x = donor;
        else if (acceptorExclusionVector[acceptor].y == -1)
            acceptorExclusionVector[acceptor].y = donor;
            ...
            throw OpenMMException("CustomResiduePairForce: this platform does not support more than four exclusions per acceptor");
    }
    donorExclusions.initialize<mm_int4>(cc, numDonors, "customResiduePairDonorExclusions");
    acceptorExclusions.initialize<mm_int4>(cc, numAcceptors, "customResiduePairAcceptorExclusions");
    donorExclusions.upload(donorExclusionVector);
    acceptorExclusions.upload(acceptorExclusionVector);
    ...
    // Now to generate the kernel.  First, it needs to calculate all distances, angles,
    // vectorangles, and dihedrals the expression depends on.
  ...
  index = 0;
    ...
    defines["M_PI"] = cc.doubleToString(M_PI);
    defines["THREAD_BLOCK_SIZE"] = "64";
    if (force.getNonbondedMethod() != CustomResiduePairForce::NoCutoff) {
        defines["USE_CUTOFF"] = "1";
        defines["CUTOFF_SQUARED"] = cc.doubleToString(force.getCutoffDistance()*force.getCutoffDistance());
    }
    if (force.getNonbondedMethod() != CustomResiduePairForce::NoCutoff && force.getNonbondedMethod() != CustomResiduePairForce::CutoffNonPeriodic)
        defines["USE_PERIODIC"] = "1";
    if (force.getNumExclusions() > 0)
        defines["USE_EXCLUSIONS"] = "1";
